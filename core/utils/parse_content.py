...
class FieldDefinition:
    def __init__(self, field: str, labels: List[str], regex: List[str], process: Callable, is_multiline: bool = False):
        self.field = field
        self.labels = labels
...
def process_languages(match, data, buffer=None):
    ...
    for language in languages:
        ...
        language_name = ''.join([char for char in language if not is_emoji(char)]).replace('#', '').strip()
        if language_name:
...
def process_subtitles(match, data, buffer=None):
    ...
    for subtitle in subtitles:
        ...
        subtitle_language = ''.join([char for char in subtitle if not is_emoji(char)]).replace('#', '').strip()
...
field_definitions = [
    FieldDefinition('title', ['📺', 'Título:'], [r'^.*?(?:📺|Título:)\s*(.*?)(?:\s*[-—:]?\s*#(\d{4})y?)?$'],
                    process_title),
    FieldDefinition('country_of_origin', ['País de Origem:', '📍 País de Origem:', 'Pais de Origem:'],
                    [r'^.*?Pa[íi]s de Origem:\s*(.*)$'], process_country_of_origin),
    FieldDefinition('directors', ['Direção:', 'Diretor:', '👑 Direção:', '👑 Direção/Roteiro:'],
                    [r'^.*?(?:Direção|Diretor|Direção\/Roteiro):\s*(.*)$'], process_directors),
    FieldDefinition('writers', ['Roteiro:', 'Roteirista:', 'Roteiristas:', '✏️ Roteirista:', '✏️ Roteiristas:'],
                    [r'^.*?(?:Roteiro|Roteirista|Roteiristas):\s*(.*)$'], process_writers),
    FieldDefinition('cast', ['Elenco:', '✨ Elenco:'], [r'^.*?Elenco:\s*(.*)$'], process_cast),
    FieldDefinition('languages', ['Idioma:', 'Idiomas:', '📣 Idiomas:', '💬 Idiomas:'],
                    [r'^.*?(?:Idiomas?|Idioma):\s*(.*)$'], process_languages),
    FieldDefinition('subtitles', ['Legenda:', 'Legendado:', '💬 Legendado:'], [r'^.*?(?:Legenda|Legendado):\s*(.*)$'],
                    process_subtitles),
    FieldDefinition('genres', ['Gênero:', 'Gêneros:', '🎭 Gêneros:'], [r'^.*?(?:Gêneros?|Gênero):\s*(.*)$'],
                    process_genres),
    FieldDefinition('synopsis', ['Sinopse', '🗣 Sinopse:', '🗣 Sinopse'], [r'^.*?(?:Sinopse|🗣 Sinopse)[:：]?\s*(.*)$'],
                    lambda match, data, buffer: process_multiline(match, data, buffer), is_multiline=True),
    FieldDefinition('curiosities', ['Curiosidades:', '💡 Curiosidades:'], [r'^.*?Curiosidades[:：]?\s*(.*)$'],
                    lambda match, data, buffer: process_multiline(match, data, buffer), is_multiline=True),
]
...
def parse_message_content(content: str) -> MovieData:
    ...
    end_of_field_markers = [
        '▶', '▶️', 'Para outros conteúdos', '💡 Curiosidades:', '🥇 Prêmios:', '🥈 Prêmios:', 'Prêmios:',
        'Clique Para Entrar',
        '🚨 Para outros conteúdos', '📣 Idiomas:', '💬 Legendado:', '📣', '💬', '#', '✨ Elenco:'
    ]
    ...
    for line in lines:
        ...
        if current_field:
            is_new_field = any(line_starts_with_label(line, field_def.labels) for field_def in field_definitions)
            is_end_of_field = any(line_starts_with_label(line, [marker]) for marker in end_of_field_markers)
