...
class APITests(SequentialTaskSet):
    ...
    def paginate_posts(self):
        ...
        with self.client.get("/api/v1/posts", params={"per_page": 10, "offset_id": 0}, catch_response=True) as response:
            if response.status_code == 200:
                response.success()
                logging.info("Paginação de posts sucedida.")
            else:
                logging.error(f"Falha na paginação de posts: {response.status_code}, {response.text}")
    ...
    def get_image(self):
        ...
        with self.client.get(f"/api/v1/posts/images/{message_id}", catch_response=True) as response:
            if response.status_code == 200:
                response.success()
                logging.info(f"Imagem com message_id={message_id} obtida com sucesso.")
            else:
                logging.error(f"Falha ao obter imagem: {response.status_code}, {response.text}")
    ...
    def stream_video(self):
        ...
        with self.client.get(
            ...
            if response.status_code == 206:
                ...
                logging.error(f"Falha no streaming de vídeo: {response.status_code}, {response.text}")
    ...
    def get_post(self):
        ...
        message_id = 7188  # Use o ID de uma mensagem existente
        with self.client.get(f"/api/v1/posts/{message_id}", headers={"Accept": "application/json"},
                             catch_response=True) as response:
            if response.status_code == 200:
                response.success()
                logging.info(f"Post com message_id={message_id} obtido com sucesso.")
            else:
                logging.error(f"Falha ao obter post: {response.status_code}, {response.text}")
